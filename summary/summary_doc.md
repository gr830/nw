...
Благодаря CSS-in-JS мы можем пробрасывать данные в наши стили и обрабатывать их, используя все возможности JS. Это даёт нам большие возможности для кастомизации уже существующих компонентов.
...
как в CSS настроено свойство box-sizing. Оно задаёт арифметику блоков и принимает одно из двух значений:
- content-box — значение по умолчанию, при котором общая ширина блока равна сумме трёх параметров: width + padding + border. 
- border-box — ширина блока равна значению width. Поля и границы включены в эту ширину: они разместятся на блоке за счёт его контента, но не повлияют на ширину самого блока. 
...
Вертикальные отступы соседних блоков схлопываются и образуют единый отступ. Сформулируем правила схлопывания. Пусть a и b — это соприкасающиеся отступы двух блоков. Тогда:
- Если a и b — положительные числа, то при схлопывании единый отступ примет значение большего из них
- Например, при отступах а = 70; b = 110 получаем при схлопывании 110 пикселей.
...
Когда разработчик указывает только часть свойств блока, браузер рассчитывает остальные самостоятельно. 
...
Когда браузер видит описание только левого поля, он считает, что противоположное поле равно нулю

Далее он рассчитывает размер левого поля. Браузер использует равенство, в котором слева будут свойства внутреннего блока, а справа — внешнего:

width + margin-left + margin-right + border + padding = width;

500 + 0 + margin-left + 0 + 0 = 666;

margin-left = 166.

## Шаблонизация

Представьте, если бы вы могли создать DOM-дерево, не используя напрямую createElement и описание свойств, а, например, вот так
```html
...
Применение такого подхода было бы в этом случае примерно таким
```js
const tmpl = `
	<div class="{{ wrapperClassName }}">
		...
...
document.body.appendChild(chats); // Вот здесь мы вставили ровно ту же вёрстку, что в начале урока. Но там было намного больше кода.
```

Никто не пишет большие и сложные приложения, используя только DOM API. У всех есть либо свои обёртки над ним, либо готовые решения, чтобы можно было «наглядно» создавать интерфейсы. Вот такие «обёртки» и называются шаблонизаторами.

**Под капотом шаблонизация**

Вы могли слышать о JSX. Он используется в React

На вход шаблонизатору подаётся обычная строка и контекст. Под контекстом подразумевается набор информации, который подставляется вместо переменных в шаблонной строке

Так шаблонизатор получает строку и объект с контекстом.

```jsx
'<ul>text</ul>'.match(/<(.*?)>/gi); // ['<ul>', '</ul>']
```

В данном примере с помощью регулярного выражения нашли имена тегов. Но можно находить как значения внутри тега, так и атрибуты со значениями.

У шаблонизаторов есть несколько режимов работы
- На вход подаём строку определённого формата и контекст для этой строки, а на выходе получаем строку с разметкой, которую можно вставить через свойство элементов innerHTML
- На выходе получаем готовый JavaScript код с набором команд для создания DOM-дерева такой же структуры, как мы передали в шаблонной строке. Данный способ намного безопаснее, но реализовать его сложнее, чем просто заменить несколько строк регулярным выражением

Реализаций шаблонизаторов много. Можно написать свой для получения огромного дополнительного опыта, можно взять готовый. Но самое главное — понимать, что внутри никакая не магия, а обычный, знакомый, а главное мощный JavaScript.

**Готовимся писать шаблонизатор**

Создадим файл block.tmpl.js. Почему *.tmpl.js? Потому что такие файлы проще отличить от обычных JS-файлов
...
Шаблон — это обычная JavaScript строка, которая возвращается из IIFE-модуля.

Почему не { content }? Потому что можно легко спутать с синтаксисом языка или, например, с объектами, шаблонными строками, интерфейсами. Вероятность встретить {{ }} гораздо ниже.

**Подготовим утилиту обработки объектов**
...
Зачем нужна эта функция? В строке-шаблоне используется получение данных из объекта span>{{ user.info.firstName }} /span

А как можно пройтись по всем ключам, которые написаны через точку
```jsx
const keys = 'a.b.c'.split('.'); // ['a', 'b', 'c']
keys.reduce((result, key) => `${result}-${key}`, ''); // '-a-b-c'
```

Соединим знания выше в одну функцию:
```jsx
...
get(obj, 'user.info.contacts.email'); // Uncaught TypeError: Cannot read property 'email' of undefined
```

Если пытаемся получить что-то от undefined, как в последнем примере, — упадём с ошибкой. Не хотелось бы, а значит, добавим в функцию обработку этого случая.Теперь всё в порядке. У пользователя не будет белого экрана от ошибки получения свойства у объекта
```jsx
function get(obj, path) {
	const keys = path.split('.');
...
Но хочется ещё пробрасывать значение по умолчанию, если желаемый ключ не найден. Когда это может быть нужно? Например, мы ждём, что значением ключа будет Boolean. В таком случае — можем вернуть false по умолчанию.
```jsx
...
//прокинули defaultValue ,если нет объекта то будет true например
get(obj, 'user.isAdmin', true); // false
```
Ура! Теперь мы можем работать с объектами. В следующем уроке начнём писать сам шаблонизатор.

**Пишем шаблонизатор**

В результате хотелось бы получить похожую сигнатуру на клиенте
```jsx
const source = "<div>{{text}}</div>";
const template = new Template(source);

// <div>Моя строка</div>
const element.innerHTML = template.compile({text: 'Моя строка'})
```

А если вам достаточно такого формата и не нужны сложные обработчики, то можно сразу добавить работу с функциями
```jsx
const source = "<div onclick='{{handleClick}}'>{{text}}</div>";
function handleClick() {
  console.log('here');
}
const template = new Template(source);

// <div onclick='handleClick()'>Моя строка</div>
const element.innerHTML = template.compile({text: 'Моя строка', handleClick})
```

Так для чего вам может пригодиться шаблонизатор? Если коротко, то для генерации DOM-дерева с подставленными атрибутами, текстами и другим статическим контентом. Полезно и экономит время. Обратите внимание, что в шаблонизатор не следует добавлять, например, работу с событиями или какую-либо другую логику

**Напишем сам шаблонизатор**

Реализуем метод compile

От слов к делу, то есть к коду. Он должен по специальному регулярному выражению получать значения внутри шаблонных обозначений — {{ }}.

Пример такого регулярного выражения — /\{\{(.*?)\}\}/gi. Ключи g и i необходимы, чтобы пройтись по всему тексту регистронезависимо.
```jsx
let testTempl = `
<div>
	{{ field1 }}
...
//если в цикле вместо константы будет сразу регулярное выражение, у вас будет каждый раз новая ссылка на него. Получите бесконечный цикл
...
while ((key = TEMPLATE_REGEXP.exec(testTempl))) {
    ...
    //убираем пробелы лишние  
		const tmplValue = key[1].trim();

		// get — функция, которую написали в предыдущем уроке
...
Теперь у нас есть механизм «компиляции» шаблонов в строку. Добавим логику к классу
```jsx
class Templator {
    TEMPLATE_REGEXP = /\{\{(.*?)\}\}/gi;
    ...
    //Почему метод compile возвращает _compileTemplate? Это сделано для того, чтобы к нему был доступ извне
    //передаем шаблон в ctx
...
Остался последний штрих — обернём всё в «модуль»
...
**Добавляем «лёгкий» обработчик**

Простые обработчики можно добавить в DOM так:
```jsx
...
Например, мы передали в контексте {handleClick: () => {}} для шаблона button onclick='{{handleClick}}', а на выходе получили строку:
```html
...
while ((key = regExp.exec(tmpl))) {
          ...
          //Мы подставляем имя функции и добавляем к ней вызов.
...
Модули в JavaScript — функционально законченный фрагмент программы, оформленный в виде отдельного файла с исходным кодом.
...
Но вместо двух надписей на странице, разработчика ждёт только сообщение об ошибке.Cкрипты содержат идентичные названия переменных, да ещё и с ключевым словом const.

Решение проблемы — замыкание.
Под замыканием понимаем лексическое окружение функции и принципы работы языка с этим лексическим окружением.
IIFE поможет решить проблему
```jsx
// file1.js
'use strict';
(function () {
...
По умолчанию браузер подключает скрипты последовательно,нужно соблюдать порядок подключения.

**Импорты ES6**

- export помечает, какие фрагменты кода должны быть доступны вне модуля;
- import подключает части модуля, размеченные через export.

Браузеры.
В браузерах модули подключаются также через тег script, но с типом подключения — module.
```html
<script src="file.js" type="module"></script
```

Особенности модулей ES2015+:
- В модулях всегда действует режим use strict;
- У каждого модуля своя изолированная область видимости;
- Код в модуле выполняется один раз за один импорт;
- В модулях нельзя обратиться к глобальному объекту, в том числе через this:
```html
<script>
...
Есть два файла: file1.js и file2.js, в которых реализованы некие функции. Хотелось бы результат первой использовать во второй и наоборот. Казалось бы, что может пойти нет так? Ответ: всё.
...
Многие браузеры поддерживают модули. 

Источник: https://caniuse.com/

**Импорты в NodeJS**

- Можно импортировать хоть классы (сервис, контроллер), хоть функции (express).
...
NodeJS — интерпретатор JavaScript, основанный на движке V8. Тот же движок работает в браузерах на базе Chromium — он транслирует JavaScript в машинный код.

Запустить скрипт можно через команду node
- node script.js 

Npm — это пакетный менеджер JavaScript.Этот менеджер по умолчанию устанавливается вместе с NodeJS.